          ...
          private bool Search(TravelListDTO _travelResponse)
            {
              if (string.IsNullOrWhiteSpace(_searchString)) return true;
                foreach (var item in _travelListDTO) {
                    if (item.Departure?.Contains(_searchString, StringComparison.OrdinalIgnoreCase) == true)
                    {
                        return true;
                    }
                }
                foreach (var item in _travelListDTO)
                {
                    if (item.Arrival?.Contains(_searchString, StringComparison.OrdinalIgnoreCase) == true)
                    {
                        return true;
                    }
                }
